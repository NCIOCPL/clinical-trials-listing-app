	...
	return (
		<>
			<FilterGroup title="Location by Zip Code">
				<input id="zip-code-filter" name="zip-code" aria-label="Enter ZIP code" type="text" className="usa-input form-control" placeholder="Enter U.S. Zip Code" value={zipCode} onChange={onZipCodeChange} maxLength={5} />
				{error && (
			...
			<FilterGroup title={FILTER_CONFIG.radius.title}>
				<div className="usa-combo-box">
					<select id="radius-filter" name="radius" aria-label="Select search radius" className="usa-select usa-combo-box__select form-control" value={radius || (zipCode ? '100' : '')} onChange={onRadiusChange} disabled={!zipCode}>
